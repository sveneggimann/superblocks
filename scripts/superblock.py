...
for city in cities:
    ...
    if cleaning_and_simplifying:
        ...
        gdf_roads.to_file("C:/_scrap/first.shp")
    ...
    for crit_deviation in crit_deviations:
        ...
        if write_interventions and crit_deviation == write_out_scenario:
            ...
            gdf_street_areas_all = gpd.GeoDataFrame(crs=city_blocks.crs)
            intersect_buildings = gpd.GeoDataFrame(crs=city_blocks.crs)
            all_blocks = gpd.GeoDataFrame(crs=city_blocks.crs)
            blocks_no_street_all = gpd.GeoDataFrame(crs=city_blocks.crs)
